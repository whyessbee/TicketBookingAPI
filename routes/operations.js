  ...
  Ticket.find({ _id: req.params.ticketid }).then((data) => {
    if (data.length) {
      User.find({ _id: data[0].passenger }).then((data) => {
        ...
        res.status(200).json({'Result':'No User Found For This Ticket Id: '+req.params.ticketid});
      }).catch((err)=>{
        next(err)});
    } else res.status(200).json({ result: "No Ticket Found With Id: "+req.params.ticketid });
  }).catch((err)=>{
    next(err)});
});
